...
export default function CarouselContent<T>({
  layoutData,
}: {
  layoutData: layoutDataType<T>;
}) {
  ...
  const cardsRef = useRef<cardDataType[]>(
    (Array.from({ length: cardCount }) as HTMLDivElement[]).map((_, i) => {
      let index =
        (i - dragDataRef.current.midIndex + layoutData.contentList.length) %
        layoutData.contentList.length;
      if (index < 0) {
        index = index + layoutData.contentList.length;
      }
  ...
  useEffect(() => {
    ...
    dragDataRef.current = {
      ...
      viewR: cardCount * layoutData.snapSize,
      midIndex: 2,
      order: Array.from({ length: cardCount }).map((_, i) => i),
      isResizing: true,
    };
    dragX.set(layoutData.baseX);

    if (cardsHolderRef.current) {
      (Array.from(cardsHolderRef.current.children) as HTMLLIElement[]).forEach(
        (v, i) => {
          cardsRef.current[i].card = v;
          const offset = i * layoutData.snapSize;
          cardsRef.current[i].offset = offset;
          v.style.translate = `${offset}px 0px`;
          // Reset the index of the card to the state before the size change so that the resized view keep card orders consistent.
          cardsRef.current[i].index =
            (viewContentIndex + i - 2 + layoutData.contentList.length) %
            layoutData.contentList.length;
        }
      );
    }
    ...
    if (containerRef.current) {
      ...
      const onWheel = (e: WheelEvent) => {
        e.preventDefault();
        if (dragDataRef.current.isResizing) return;
        if (e.deltaX != 0) {
      ...
      if (offsetCount > 0) {
        Array.from({ length: offsetCount }).forEach(() => {
          if (offset < 0) {
            ...
            const newIndex =
              (cards[cardsOrder[0]].index - 1 + layoutData.contentList.length) %
              layoutData.contentList.length;
            setViewContentIdex(
              (prev) =>
  ...
  return (
    <>
      ...
      >
        {layoutData.contentList.map((_, i) => (
          <Dot
            key={i}
            index={i}
            onScroll={onScroll}
